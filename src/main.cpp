...
int main(int argc, char **argv) {
    ...
    options.custom_help("[options]\n\n"
                        "Smoothpaper is configured with a configuration file at\n\n"
                        "    ~/.config/smoothpaper/smoothpaper.toml\n\n"
                        "You can install a default configuration file with the -i option.\n"
                        "Please see that file for configuration options.");
    ...
    if (args["installconfig"].as<bool>()) {
        auto        xdg_config_home = std::getenv("XDG_CONFIG_HOME");
        std::string config_dir;
        if (xdg_config_home == nullptr) {
            auto home  = std::string(std::getenv("HOME"));
            config_dir = fmt::format("{}/.config", home);
        } else {
            config_dir = xdg_config_home;
        }

        auto        fs        = cmrc::conf::get_filesystem();
        auto        conf      = fs.open("smoothpaper.toml");
        auto        conf_data = conf.begin();
        auto        conf_end  = conf.end();
        std::string conf_str;
        for (; conf_data != conf_end; ++conf_data) {
            conf_str += *conf_data;
        }

        if (std::filesystem::exists(fmt::format("{}/smoothpaper/smoothpaper.toml", config_dir))) {
            spdlog::error("Error: config file already exists at {}/smoothpaper/smoothpaper.toml", config_dir);
            return EXIT_FAILURE;
        }

        std::filesystem::create_directories(fmt::format("{}/smoothpaper", config_dir));
        std::ofstream out(fmt::format("{}/smoothpaper/smoothpaper.toml", config_dir));
        out << conf_str;
        out.close();

        spdlog::info("installed default config file at {}/smoothpaper/smoothpaper.toml", config_dir);
    ...
    black_sprite.setScale(
        static_cast<float>(render_window->getSize().x), static_cast<float>(render_window->getSize().y));
